...
struct _Func<F, R, T1, NoType> : _FuncB<R, T1, NoType>
{
	_Func(const F& f) : f(f) {}
...
struct _Func<F, R, NoType, NoType> : _FuncB<R, NoType, NoType>
{
	_Func(const F& f) : f(f) {}
	R operator()() { return f(); }
	F f;
...
#define ASL_FUNC(T1, T2) \
	...
	template<class F> \
	Function(const F& f) : f(new _Func<F, R, T1, T2>(f)) {} \
	~Function() { delete f; } \
	_FuncB<R, T1, T2>* f;
